...
同等blockの2回目、またはtotal recovery budget消費時は `handoff_required` を返し、Jev mutationを停止します。Jevのmodel-call budget到達も例外のtool errorにはせず、blockedとして同じ上限付きRecovery経路へ入れます。
